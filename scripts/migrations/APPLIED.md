...
- **Bloqueante — un `EXISTS` inline contra `roles_usuario` dentro de una policy de OTRA
  ...
  consultando las filas de `roles_usuario` de su barbero veía CERO filas siempre —
  confirmado con un `UPDATE 0` real al probar "Ana bloquea a su barbero Carlos". No es una
...
`bloqueado_hasta` propio; `dueno_sede` bloqueando a su barbero (el caso que expuso el primer
...
1. **Bloqueante (multi-tenant-guard) — edición sin restricción de columnas en el perfil de
   ...
   otras sedes tiene actividad ese mismo usuario; (c) el trigger de columnas de la primera
...
2. **Bloqueante nuevo — `bloqueado_hasta` habilitaba sabotaje cross-tenant real entre
   ...
   bloquea la cuenta GLOBALMENTE (las 20 sedes, no por tenant), estaba en el allow-list de
   esa rama. Resultado: un `dueno_sede` de la sede A podía "atar" como cliente suyo al
   `dueno_sede`/`barbero` de la sede B con solo conocer su `usuario_id`/correo, y bloquearlo
   de operar en TODA la plataforma — sabotaje directo entre competidores, no cubierto por la
   decisión de producto ya aceptada (esa solo hablaba de 3 campos de LECTURA, no de
   escritura con efecto global). No estaba cubierto por ningún escenario anterior: todos
   probaban relaciones DENTRO del propio tenant o el caso de un usuario multi-tenant
   (Carlos), pero ninguno simulaba a un `dueno_sede`/`barbero` de OTRA sede siendo "atado"
   como cliente trivial.
   ...
   **Decisión de diseño evaluada y tomada** (una de las dos direcciones que propuso el
   ...
   poder aplicar sobre un cliente/barbero problemático de SU sede **ya existía con el
   alcance correcto**: `clientes.activo`/`barberos.activo` (columnas tenant-scoped,
   `003_barberos.sql`/`005_clientes.sql`, RLS estándar por `tenant_id` de la fila) — no hizo
   falta una tabla ni columna nueva, solo sacar `bloqueado_hasta` del perfil transversal. Se
   evaluó también la segunda dirección (restringir `roles_usuario_insert` para que staff no
   pueda atar como `cliente` walk-in a un `usuario_id` que ya tiene rol
   `dueno_sede`/`administrador_plataforma`/`barbero` en cualquier tenant) — **se descartó**
   por ahora: con `bloqueado_hasta`/`eliminado_at` ya exclusivos de
