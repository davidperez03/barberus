...
-- auth.users (no duplicada por tenant en clientes/profesionales/dueños) — ver esa migración para
...
--     para profesional/dueno_sede, normal para cliente): GoTrue configura el JWT/refresh token
...
--     SOLO cuando el rol activo de la sesión es profesional/dueno_sede (se resuelve leyendo
...
--     request a la escala de los negocios de la plataforma, contradice la guía de performance de
...
--   - staff de la sede (dueno_sede o profesional) puede registrar la cuenta de un cliente
...
--     un profesional en SU sede — un profesional no puede contratar a otro profesional.
...
create policy roles_usuario_insert on public.roles_usuario
  ...
  with check (
    ...
    or (rol = 'profesional' and public.es_dueno_del_tenant(tenant_id))
...
-- revocar profesional/cliente de SU propia sede (nunca una fila dueno_sede/administrador_plataforma
...
create policy roles_usuario_delete on public.roles_usuario
  ...
  using (
    ...
    or (rol in ('cliente', 'profesional') and public.es_dueno_del_tenant(tenant_id))
  );
...
comment on policy roles_usuario_insert on public.roles_usuario is
  'Cliente se autoasigna su propio rol cliente; staff/dueno_sede/administrador_plataforma asignan según la tabla de alcance en auth-users.md. Nunca autoservicio para dueno_sede/administrador_plataforma/profesional.';
comment on policy roles_usuario_delete on public.roles_usuario is
  'administrador_plataforma revoca cualquier rol; dueno_sede solo revoca profesional/cliente de su propia sede. Sin política de UPDATE a propósito (ver comentario en la migración).';

-- Índice de soporte para lookups de "todas las filas de cliente de este usuario_id, en
-- cualquier sede" (p.ej. el propio usuario viendo sus perfiles en distintos negocios) --
...
--    política de UPDATE -- profesionales_update NO tiene ese branch (solo dueno_sede edita
--    profesionales hoy, ver 003_profesionales.sql), así que no hay autoservicio que restringir ahí.
-- ============================================================================
